...
int main() {
    ...
    for (int t = 0; t < numStrings - 1; t++) {
        ...
        // this part is slow
        // maybe try skipping strings where they are part of chains (check it's count value)
        ...
        while (true) {            
            ...
            // cout << "here" << endl;
            // do it once if counter is 0, do it twice if counter is 1 (skip a chain piece)
            int skips = counters[currentNode->data];
            for (int i = 0; i < skips; i++) {
                // cout << "data: " << strings[currentNode->data] << endl;
                // cout << (aNode != nullptr) << (bNode != nullptr) << endl;
                // cout << "i is: " << i << endl;
                // if ()
                // the problem is beforeB is wrong now
