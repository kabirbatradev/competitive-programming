...
void getNumCombinations(bool hasL, vector<int> intString, long long combinationsSoFar) {
...
int main() {
  ...
  // first check for combos, then do the _s stuff
  ...
  int numUnderscores = count(intString.begin(), intString.end(), 2);
  if (numUnderscores == 0) {
    // if no nunderscores and no L, then bad
    if (!hasL) {
      cout << 0 << endl;
      return 0;
    } else {
    ...
    // there are underscores, there might be L, there are no combos
