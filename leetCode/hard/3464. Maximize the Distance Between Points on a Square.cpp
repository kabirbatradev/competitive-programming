...
public:
  ...
  bool canFunction(
      ...
      // Create extended thetas array
      vector<double> extended_thetas(2 * n);
      for(int i = 0; i < n; i++) {
          extended_thetas[i] = thetas[i];
          extended_thetas[i + n] = thetas[i];
      }
      ...
      // for (int i = 0; i < n; i++) { 
          int start = head->next->index;
          // if (visited[i]) continue;
          // visited[i] = true;
          removeFromLinkedList(mapping[start]);
          // i is starting theta

          int pointsCounter = 1;
          pair<int,int> startPoint = m[thetas[start]];
          pair<int,int> lastPoint = startPoint;

          // Use extended array for searching
          int curr = start;
          while(pointsCounter < k) {
              // Binary search in extended array
              int next = -1;
              int low = curr + 1;
              int high = curr + n - 1;
              ...
              while(low <= high) {
                  ...
                  pair<int,int> midPoint = m[extended_thetas[mid]];
                  int distance = dist(midPoint, lastPoint);
                  
                  if(distance >= threshold) {
                      next = mid % n;  // convert back to original array index
                      high = mid - 1;
                  } else {
                      low = mid + 1;
                  }
              ...
              // Check if too close to start point
              if(dist(m[thetas[next]], startPoint) < threshold) {
                  break;
              }
